import { createIcon } from '@chakra-ui/icons';

export const FirstDishesIcon = createIcon({
    displayName: 'FirstDishesIcon',
    viewBox: '0 0 24 24',
    path: (
        <>
            <rect width='24' height='24' fill='url(#pattern0_5002_136563)' />
            <defs>
                <pattern
                    id='pattern0_5002_136563'
                    patternContentUnits='objectBoundingBox'
                    width='1'
                    height='1'
                >
                    <use
                        xlinkHref='#image0_5002_136563'
                        transform='translate(0.166667 0.166667) scale(0.00694444)'
                    />
                </pattern>
                <image
                    id='image0_5002_136563'
                    width='96'
                    height='96'
                    preserveAspectRatio='none'
                    xlinkHref='data:image/png;base64,'
                />
            </defs>
        </>
    ),
});
